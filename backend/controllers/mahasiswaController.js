...
exports.getAll = async (req, res) => {
  try {
    ...
    console.error(error);
    res.status(500).json({ message: 'Gagal memuat mahasiswa' });
  }
};

exports.getById = async (req, res) => {
  try {
    const [rows] = await pool.query('SELECT m.*, u.nama, u.email, u.foto FROM mahasiswa m JOIN users u ON m.user_id = u.id WHERE m.id = ? AND u.role = ?', [req.params.id, 'mahasiswa']);
...
exports.create = async (req, res) => {
  ...
  if (!nim || !nama || !email) return res.status(400).json({ message: 'NIM, nama, email wajib' });
  ...
  try {
    ...
    const [userRes] = await conn.query('INSERT INTO users (nama, email, password, role) VALUES (?,?,?,?)', [nama, email, hashedPassword, 'mahasiswa']);
    await conn.query('INSERT INTO mahasiswa (user_id, nim, prodi, angkatan, no_hp, status_magang) VALUES (?,?,?,?,?,?)', [userRes.insertId, nim, prodi, angkatan || new Date().getFullYear(), no_hp, status_magang || 'Pending']);
    await conn.commit();
...
exports.update = async (req, res) => {
  ...
  try {
    await pool.query('UPDATE mahasiswa SET prodi=?, angkatan=?, no_hp=?, status_magang=? WHERE id=?', [prodi, angkatan, no_hp, status_magang, id]);
...
exports.remove = async (req, res) => {
  ...
  const conn = await pool.getConnection();
  try {
    const [mhs] = await conn.query('SELECT m.user_id, u.role FROM mahasiswa m JOIN users u ON m.user_id = u.id WHERE m.id=?', [id]);
