...
exports.getAll = async (req, res) => {
  try {
    ...
    const [rows] = await pool.query(`SELECT * FROM perusahaan ${where} ORDER BY id DESC LIMIT ? OFFSET ?`, [...params, parseInt(limit), parseInt(offset)]);
...
exports.create = async (req, res) => {
  try {
    ...
    await pool.query('INSERT INTO perusahaan (nama_perusahaan, alamat, kota, bidang_usaha, email, telepon, pic) VALUES (?,?,?,?,?,?,?)', [nama_perusahaan, alamat, kota, bidang_usaha, email, telepon, pic]);
    res.status(201).json({ message: 'Perusahaan ditambahkan' });
  } catch (error) {
    console.error(error);
...
exports.update = async (req, res) => {
  try {
    const { id } = req.params;
    const { nama_perusahaan, alamat, kota, bidang_usaha, email, telepon, pic } = req.body;
    await pool.query('UPDATE perusahaan SET nama_perusahaan=?, alamat=?, kota=?, bidang_usaha=?, email=?, telepon=?, pic=? WHERE id=?', [nama_perusahaan, alamat, kota, bidang_usaha, email, telepon, pic, id]);
